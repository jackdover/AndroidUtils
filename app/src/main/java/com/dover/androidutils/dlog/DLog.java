...
public class DLog {
    ...
    private static class TagHead {
        ...
        TagHead(String tag, String[] consoleHead, String fileHead) {
            this.tag = TextUtils.isEmpty(sGlobalTag) ? tag : sGlobalTag + tag; // 默认在所有tag 前加 appName
    ...
    public static class LogConfig {
        private LogConfig(@NonNull Context context) {
            sGlobalTag = getAppName(context);  //默认用 AppName 作为全局tag
        ...
        @NonNull
        private String getAppName(@NonNull Context context) {
            try {
